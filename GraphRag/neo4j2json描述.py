...
def fetch_all_relationships():
    ...
    query = """
    MATCH (n)-[r]->(m)
    RETURN n.name AS start_node, type(r) AS relation, m.name AS end_node, n.description AS start_description, m.description AS end_description
    """
    ...
    with driver.session() as session:
        ...
        for record in results:
            ...
            start_description = record.get("start_description", "")
            end_description = record.get("end_description", "")
            ...
            if start_node not in nodes_map:
                ...
                nodes_list.append({
                    "id": node_id,
                    "name": start_node,
                    "content": start_description,  # 新增 content 字段
                })
                node_id += 1
            ...
            if end_node not in nodes_map:
                ...
                nodes_list.append({
                    "id": node_id,
                    "name": end_node,
                    "content": end_description,  # 新增 content 字段
                })
                node_id += 1
...
def main():
    ...
    save_to_json_file(graph_data, "graph_content.json")
